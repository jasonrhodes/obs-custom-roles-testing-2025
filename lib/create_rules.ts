...
import getRules, {
  CUSTOM_ROLES_LOGS_DATA_VIEW_ID,
  RuleConfig,
} from "../data/test_rules";
import kbnClient from "./kbn_client";
import { createTestMonitor } from "./create_monitor";
...
export async function createRules() {
  // create required test synthetics monitor
  const { agentPolicyId } = await createTestMonitor();
    ...
    try {
      ...
      if (response.status >= 400) {
        throw new AxiosError(
          `Custom-made axios error while creating ${rule.name}`,
